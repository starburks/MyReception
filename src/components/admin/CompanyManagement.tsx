...
import { Plus, Pencil, Trash2, Download, ArrowUpDown } from 'lucide-react';
...
const CompanyManagement: React.FC = () => {
  ...
  const handleDelete = async (company: companies) => {
    if (!window.confirm(`${company.name}を削除してもよろしいですか？`)) return;
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="space-x-4">
          <button onClick={exportToCsv} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50">
            <Download className="w-4 h-4 inline-block mr-2" />
            CSVエクスポート
          </button>
          <button onClick={() => { setIsEditing(true); setEditingCompany({}); }} className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700">
      ...
      {isEditing && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h3 className="text-lg font-medium mb-4">{editingCompany.id ? '会社情報編集' : '新規会社登録'}</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Chatwork Room ID</label>
                <input
                  type="text"
              ...
              <div className="flex justify-end space-x-3">
                <button type="button" onClick={() => { setIsEditing(false); setEditingCompany({}); }} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50">
                  キャンセル
                </button>
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th onClick={() => toggleSort('name')} className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                会社名 <ArrowUpDown className="inline w-4 h-4 ml-1" />
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Chatwork Room ID</th>
              <th onClick={() => toggleSort('created_at')} className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                作成日 <ArrowUpDown className="inline w-4 h-4 ml-1" />
              </th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {sortedCompanies.map((company) => (
              <tr key={company.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{company.chatwork_room_id || '-'}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{new Date(company.created_at).toLocaleString('ja-JP')}</td>
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <button onClick={() => { setIsEditing(true); setEditingCompany(company); }} className="text-blue-600 hover:text-blue-900 mr-3">
